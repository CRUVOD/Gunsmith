...
/// <summary>
/// This is a base class which you can use to shake, modify a value (i.e. a float) for a specified amount of time
/// Used in conjunction with the feedback system
/// </summary>
public class ValueModifier : MonoBehaviour
{
    [Header("Value Modifier Settings")]
    ...
    protected virtual float ShakeFloat(AnimationCurve curve, float remapMin, float remapMax, bool relativeIntensity, float initialValue)
    {
        ...
        float remappedTime = FeedbacksHelpers.Remap(_journey, 0f, ShakeDuration, 0f, 1f);

        float curveValue = curve.Evaluate(remappedTime);
        newValue = FeedbacksHelpers.Remap(curveValue, 0f, 1f, remapMin, remapMax);

...
public class AIState
{
    ...
    [ExtraReoderableAttribute(null, "Action", null)]
    public AIActionsList Actions;
    [ExtraReoderableAttribute(null, "Transition", null)]
    ...
    public virtual void SetCore(AICore core)
    {
        this.core = core;
    }
    ...
    public virtual void PerformActions()
    {
        ...
        for (int i = 0; i < Actions.Count; i++)
        {
            ...
            {
                Debug.LogError("An action in " + core.gameObject.name + " on state " + StateName + " is null.");
            }
        }
    }
        ...
        for (int i = 0; i < Transitions.Count; i++)
        {
            if (Transitions[i].Decision != null)
            {
                if (Transitions[i].Decision.Decide())
                {
                    ...
                    {
                        core.TransitionToState(Transitions[i].TrueState);
                        break;
                    }
                }
                else
                {
                    ...
                    {
                        core.TransitionToState(Transitions[i].FalseState);
                        break;
                    }
                }
            }

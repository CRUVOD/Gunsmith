...
public class SimpleFollow : MonoBehaviour
{
        ...
        if (InterpolatePosition)
        {
            switch (FollowPositionMode)
            {
                case FollowModes.MMLerp:
                    interpolatedDistance = ExtraMaths.Lerp(0f, trueDistance, FollowPositionSpeed, Time.deltaTime);
                ...
                case FollowModes.MMSpring:
                    ...
                    ExtraMaths.Spring(ref _newPosition, _newTargetPosition, ref _velocity, PositionSpringDamping, PositionSpringFrequency, FollowPositionSpeed, Time.deltaTime);
                    if (_localSpace)
                    {
    ...
    protected virtual void FollowTargetRotation()
    {
        ...
        if (InterpolateRotation)
        {
            ...
            {
                case FollowModes.MMLerp:
                    this.transform.rotation = ExtraMaths.Lerp(this.transform.rotation, _newTargetRotation, FollowRotationSpeed, Time.deltaTime);
                    break;
                case FollowModes.RegularLerp:
    ...
    protected virtual void FollowTargetScale()
    {
        ...
        if (InterpolateScale)
        {
            ...
            {
                case FollowModes.MMLerp:
                    this.transform.localScale = ExtraMaths.Lerp(this.transform.localScale, _newScale, FollowScaleSpeed, Time.deltaTime);
                    break;
                case FollowModes.RegularLerp:
